...
test_sensitivity_epsilon = 10 #bbble darkness error sensitivity
...
def ProcessPage(paper):
    ...
    columns = [[37.0 / scaling[0], 55 / scaling[1]], [155.0 / scaling[0], 55 / scaling[1]], [274.0 / scaling[0], 55 / scaling[1]], [393.0 / scaling[0], 55 / scaling[1]], [510.0 / scaling[0], 55 / scaling[1]]] #dimensions of the columns of bubbles
    radius = 3.5 / scaling[0] #radius of the bubbles
    spacing = [70.0 / scaling[0], 59.0 / scaling[1]] #spacing of the rows and columns
    ...
    columns = [[33.0 / scaling[0], 394 / scaling[1]], [152.0 / scaling[0], 394 / scaling[1]], [270.0 / scaling[0], 394 / scaling[1]], [389.0 / scaling[0], 394 / scaling[1]], [507.0 / scaling[0], 394 / scaling[1]]] #dimensions of the columns of bubbles
    radius = 3.5 / scaling[0] #radius of the bubbles
    spacing = [70.0 / scaling[0], 33.8 / scaling[1]] #spacing of the rows and columns 

    for k in range(0, 5): #columns
        for i in range(0, 12): #rows
            questions = []
            for j in range(0, 4): #answers
                #coordinates of the answer bubble
                x1 = int((columns[k][0] + (j/3.5)*spacing[0] - radius*2)*dimensions[0] + corners[0][0])
    ...
    if codes is not None:
        ...
        codes = codes[2:]
        codes = codes[:5]
        cv2.putText(paper, codes, (int(100), int(400)), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 0), 1)
    else:
        codes = [-1]
